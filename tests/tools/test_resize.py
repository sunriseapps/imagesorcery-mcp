...
class TestResizeToolDefinition:
    ...
    async def test_resize_in_tools_list(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            assert "resize" in tool_names, "resize tool should be in the list of available tools"
    ...
    async def test_resize_description(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            assert "resize" in resize_tool.description.lower(), "Description should mention that it resizes an image"
    ...
    async def test_resize_parameters(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            # Check input schema
            assert hasattr(resize_tool, 'inputSchema'), "resize tool should have an inputSchema"
            assert 'properties' in resize_tool.inputSchema, "inputSchema should have properties field"
            
            # Check required parameters
            assert "input_path" in resize_tool.inputSchema['properties'], "resize tool should have an 'input_path' property in its inputSchema"
            
            # Check optional parameters
            optional_params = ["width", "height", "scale_factor", "interpolation", "output_path"]
            for param in optional_params:
                assert param in resize_tool.inputSchema['properties'], f"resize tool should have a '{param}' property in its inputSchema"
            
                # Check parameter types - accounting for optional parameters that use anyOf structure
                assert resize_tool.inputSchema['properties']['input_path'].get('type') == 'string', "input_path should be of type string"
                ...
                for param in ['width', 'height']:
                    ...
                    if 'anyOf' in param_schema:
                        ...
                        has_integer_type = any(option.get('type') == 'integer' for option in param_schema['anyOf'])
                        assert has_integer_type, f"{param} should allow integer type"
                    else:
                        assert param_schema.get('type') == 'integer', f"{param} should be of type integer"
        
                # For scale_factor (float parameter)
                scale_factor_schema = resize_tool.inputSchema['properties']['scale_factor']
                if 'anyOf' in scale_factor_schema:
                    # Check if one of the anyOf options is number
                    has_number_type = any(option.get('type') == 'number' for option in scale_factor_schema['anyOf'])
                    assert has_number_type, "scale_factor should allow number type"
                else:
                    assert scale_factor_schema.get('type') == 'number', "scale_factor should be of type number"
                ...
                for param in ['interpolation', 'output_path']:
                    ...
                    if 'anyOf' in param_schema:
                        ...
                        has_string_type = any(option.get('type') == 'string' for option in param_schema['anyOf'])
                        assert has_string_type, f"{param} should allow string type"
                    else:
                        assert param_schema.get('type') == 'string', f"{param} should be of type string"
class TestResizeToolExecution:
    """Tests for the resize tool execution and results."""

    @pytest.mark.asyncio
    async def test_resize_with_dimensions_smaller(self, mcp_server: FastMCP, test_image_path, tmp_path):
        """Tests the resize tool execution with specific dimensions (smaller)."""
        output_path = str(tmp_path / "output_dimensions_smaller.png")
        
        async with Client(mcp_server) as client:
            result = await client.call_tool("resize", {
    ...
    @pytest.mark.asyncio
    async def test_resize_with_dimensions_larger(self, mcp_server: FastMCP, test_image_path, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_with_width_only_smaller(self, mcp_server: FastMCP, test_image_path, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_with_width_only_larger(self, mcp_server: FastMCP, test_image_path, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_with_height_only_smaller(self, mcp_server: FastMCP, test_image_path, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_with_height_only_larger(self, mcp_server: FastMCP, test_image_path, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_with_scale_factor_smaller(self, mcp_server: FastMCP, test_image_path, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_with_scale_factor_larger(self, mcp_server: FastMCP, test_image_path, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_default_output_path(self, mcp_server: FastMCP, test_image_path):
    ...
    @pytest.mark.asyncio
    async def test_resize_with_interpolation(self, mcp_server: FastMCP, test_image_path, tmp_path):
