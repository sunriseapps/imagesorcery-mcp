...
class TestMetainfoToolDefinition:
    ...
    async def test_metainfo_in_tools_list(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            assert "get_metainfo" in tool_names, "get_metainfo tool should be in the list of available tools"
    ...
    async def test_metainfo_description(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            tools = await client.list_tools()
            metainfo_tool = next((tool for tool in tools if tool.name == "get_metainfo"), None)
            
            # Check description
            assert metainfo_tool.description, "get_metainfo tool should have a description"
            assert "metadata" in metainfo_tool.description.lower(), "Description should mention metadata"
    ...
    async def test_metainfo_parameters(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            # Check input schema
            assert hasattr(metainfo_tool, 'inputSchema'), "get_metainfo tool should have an inputSchema"
            assert 'properties' in metainfo_tool.inputSchema, "inputSchema should have properties field"
            
            # Check required parameters
            required_params = ["input_path"]
            for param in required_params:
                assert param in metainfo_tool.inputSchema['properties'], f"get_metainfo tool should have a '{param}' property in its inputSchema"
            
            # Check parameter types
            assert metainfo_tool.inputSchema['properties']['input_path'].get('type') == 'string', "input_path should be of type string"
...
class TestMetainfoToolExecution:
    ...
    async def test_metainfo_tool_execution(self, mcp_server: FastMCP, test_image_path):
        ...
        async with Client(mcp_server) as client:
            result = await client.call_tool("get_metainfo", {
    ...
    async def test_metainfo_nonexistent_file(self, mcp_server: FastMCP, tmp_path):
        ...
        async with Client(mcp_server) as client:
            with pytest.raises(Exception) as excinfo:
                await client.call_tool("get_metainfo", {
                    "input_path": nonexistent_path
                })
            
            # Check that the appropriate error was raised
            assert "not found" in str(excinfo.value).lower() or "FileNotFoundError" in str(excinfo.value)