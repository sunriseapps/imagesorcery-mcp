...
class TestRotateToolDefinition:
    ...
    async def test_rotate_in_tools_list(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            assert "rotate" in tool_names, "rotate tool should be in the list of available tools"
    ...
    async def test_rotate_description(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            assert "rotate" in rotate_tool.description.lower(), "Description should mention that it rotates an image"
    ...
    async def test_rotate_parameters(self, mcp_server: FastMCP):
        ...
        async with Client(mcp_server) as client:
            ...
            # Check input schema
            assert hasattr(rotate_tool, 'inputSchema'), "rotate tool should have an inputSchema"
            assert 'properties' in rotate_tool.inputSchema, "inputSchema should have properties field"
            
            # Check required parameters
            required_params = ["input_path", "angle"]
            for param in required_params:
                assert param in rotate_tool.inputSchema['properties'], f"rotate tool should have a '{param}' property in its inputSchema"
            
            # Check optional parameters
            assert "output_path" in rotate_tool.inputSchema['properties'], "rotate tool should have an 'output_path' property in its inputSchema"
            
            # Check parameter types
            assert rotate_tool.inputSchema['properties']['input_path'].get('type') == 'string', "input_path should be of type string"
            assert rotate_tool.inputSchema['properties']['angle'].get('type') in ['number', 'integer', 'float'], "angle should be a numeric type"
            assert rotate_tool.inputSchema['properties']['output_path'].get('type') == 'string', "output_path should be of type string"
...
class TestRotateToolExecution:
    ...
    @pytest.mark.asyncio
    async def test_rotate_tool_execution(self, mcp_server: FastMCP, test_image_path, tmp_path):
        """Tests the rotate tool execution and return value."""
        output_path = str(tmp_path / "output.png")
        
        async with Client(mcp_server) as client:
            result = await client.call_tool("rotate", {
            ...
            margin = 5
            assert abs(original_width - rotated_height) <= margin, "Original width should approximately match rotated height for 90-degree rotation"
            assert abs(original_height - rotated_width) <= margin, "Original height should approximately match rotated width for 90-degree rotation"
            ...
            for y in range(10, 40):
                for x in range(rotated_width - 40, rotated_width - 10):
                    if x >= 0 and x < rotated_width and y >= 0 and y < rotated_height:
                        ...
                        if pixel[0] < 50 and pixel[1] < 50 and pixel[2] > 200:  # Allow for some color variation
                            has_red_pixels = True
                            break
                if has_red_pixels:
                    break
                    
            assert has_red_pixels, "Red rectangle should be in the top-right area after 90-degree counterclockwise rotation"

    @pytest.mark.asyncio
    async def test_rotate_clockwise(self, mcp_server: FastMCP, test_image_path, tmp_path):
        ...
        async with Client(mcp_server) as client:
            ...
            assert has_red_pixels, "Red rectangle should be in the bottom-left area after 90-degree clockwise rotation"

    @pytest.mark.asyncio
    async def test_rotate_default_output_path(self, mcp_server: FastMCP, test_image_path):
