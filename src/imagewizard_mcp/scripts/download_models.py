...
def download_model(model_size):
    ...
    try:
        ...
        # If not found in expected locations, try to find it in ultralytics default location
        possible_locations = [
        ...
        for loc in possible_locations:
            if loc.exists():
                ...
                try:
                    ...
                    print(f"❌ Please manually copy the model from {loc} to {model_path}")
                    return False
...
def main():
    parser = argparse.ArgumentParser(description="Download YOLOv8 models for offline use")
    parser.add_argument(
        "--model-size", 
        choices=["n", "s", "m", "l", "x"], 
        default="m",
